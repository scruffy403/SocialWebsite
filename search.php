...
<div class="main_column column" id="main_column">
  ...
  if($query == "")
    ...
    // If there are two words, assume they are first and last names respectively
      ...
      // If query has one work only, search first names or last names
    ...
    while ($row = mysqli_fetch_array($usersReturnedQuery)) {
      ...
      if ($user['username'] != $row['username']) {
        ...
        if($user_object->isFriend($row['username']))
          $button = "<input type='submit' name='" . $row['username'] . "' class='danger' value='Remove Friend'>";
        else if($user_object->didReceiveRequest($row['username']))
          $button = "<input type='submit' name='" . $row['username'] . "' class='warning' value='Respond to request'>";
        else if($user_object->didSendRequest($row['username']))
          $button = "<input type='submit' class='default' value='Request sent'>";
        else
          $button = "<input type='submit' name='" . $row['username'] . "' class='success' value='Add Friend'>";

        $mutual_friends = $user_object->getMutualFriends($row['username']). " friends in common";

        // Button forms
        if (isset($_POST[$row['username']])) {

          if ($user_object->isFriend($row['username'])) {
            $user_object->removeFriend($row['username']);
            ...
            // Add cancel request functionality later...
          }
          else {
            $user_object->sendRequest($row['username']);
            header("Location: http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
          }
